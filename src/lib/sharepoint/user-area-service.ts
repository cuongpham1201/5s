...
export async function listUserAllowedAreas(email: string): Promise<AreaOption[]> {
  ...
  for (const r of rows) {
    ...
    if (a) out.push(a);
    else out.push({ code: r.areaCode, name: r.areaCode, departmentCode: r.departmentCode ?? "", departments: r.departmentCode ? [r.departmentCode] : [], parentCode: null, sortOrder: 0 });
  }
