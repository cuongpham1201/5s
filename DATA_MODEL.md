...
- **Lọc nguồn (active members only):** chỉ lấy user `accountEnabled=true` + `userType="Member"` + email/UPN `@biahalong.com` + có department. Loại disabled / guest / external / không có dept. (Tránh phòng ban cũ từ tài khoản vô hiệu.)
- **deactivateMissing = false (mặc định):** import KHÔNG deactivate phòng ban thiếu trong nguồn trừ khi gọi `?deactivateMissing=true`. Tránh tắt nhầm khi nguồn chưa đầy đủ.
- **Graph users source:** quét `users.department` (paginate, đã lọc active member), bỏ null/rỗng, **distinct theo tên chuẩn hóa**, gom **theo CODE** (biến thể của cùng phòng ban → 1 record). `DepartmentName` = giá trị raw (biến thể dài nhất). `DepartmentCode` = mã chính thức (bảng `org-codes.ts` `OFFICIAL_DEPARTMENTS`, vd "Ban Tài chính - Kiểm soát nội bộ" → **TCKS**), nếu chưa có → mã suy diễn (deterministic, admin sửa sau). Upsert theo `DepartmentCode`; thiếu trong nguồn → `IsActive=false` (không xoá); nguồn 0 phần tử → không deactivate.
